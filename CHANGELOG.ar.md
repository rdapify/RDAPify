...
- **توسيع قائمة حظر عناوين الـ IP (مسح المحيط منخفض الخطورة)**: صار
  `is_blocked_ip` و`SsrfGuard` يحظران أيضًا نطاق CGNAT `100.64.0.0/10`
  (RFC 6598)، وبادئتي NAT64 `64:ff9b::/96` (RFC 6052) و`64:ff9b:1::/48`
  (RFC 8215)، ونطاقات الاختبار/التوثيق `192.0.2.0/24` و`198.51.100.0/24`
  و`203.0.113.0/24` (RFC 5737). كما يُفرَض `require_https` فورًا قبل أي بحث DNS،
  وجُعِلت رسائل المضيف المحظور عامّة لتفادي تسريب طوبولوجيا الشبكة الداخلية.
